...
description: Learn who builds OpenCode Mobile, how the Android app relates to OpenCode, and where its source, releases, testing evidence, and support channels live.
last_update:
...
OpenCode Mobile is an independent, community-built Android client for the open-source [OpenCode](https://opencode.ai/) coding agent. It is not an official OpenCode product and does not replace the OpenCode server. The app connects to a server that you run and control.
...
- Builds are published through [GitHub Releases](https://github.com/alvarolorentedev/opencode-mobile/releases) and Google Play testing.
...
OpenCode runs the agents, models, tools, sessions, and workspace operations. OpenCode Mobile is a focused control surface for starting and monitoring that work from Android. It is deliberately not presented as a general chatbot, complete mobile IDE, or full terminal emulator.

Read the [Android setup guide](/docs/opencode-android-app/), review the [security model](/security/), or inspect the [current release history](/docs/changelog/).
